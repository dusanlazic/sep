...
def update_transaction_status(db: Session, statusUpdate: OrderStatusUpdateRequest):
    try:
        transaction = (
            ...
            .filter_by(psp_order_id=statusUpdate.psp_order_id)
            .one()
        )
        ...
        db.commit()
        print(
            f"Transaction with PSP Order ID {statusUpdate.psp_order_id} updated successfully."
        )
    except NoResultFound:
        print(f"Transaction with PSP Order ID {statusUpdate.psp_order_id} not found.")
    except KeyError:
        print(
